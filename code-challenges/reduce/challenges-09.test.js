...
const countNumberOfElements = arr => {
  return arr.reduce((compile, curVal) => compile + 1);
};
...
const returnNames = arr => {
  return arr.reduce((otherArr, val) => {
    otherArr.push(val.name);
    return otherArr;
  }, []);
};
...
const calculateAverage = arr => {
  return arr.reduce((firstVal, secondVal) => firstVal + secondVal) / arr.length;
};
