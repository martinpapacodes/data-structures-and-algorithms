...
const divisibleByFiveTwoToThePower = (input) => {
  let arr = [];
  input.filter(values => {
    let collectionOfValues = values.filter(value => {
      if(typeof value === 'number' && value % 5 === 0) {
        return value;
      }
    })
    let newArr = collectionOfValues.map(value => Math.pow(2, value))
    arr.push(newArr)
  })
  return arr;
};
...
let findMaleAndFemale = (data) => {
 return data.filter((gender) => {
  ...
  }).reduce((character, currentCharacter) => {
    if(typeof character === 'object') {
      return `${character.name} and ${currentCharacter.name}`;
    } else {
      return `${character} and ${currentCharacter.name}`;
    }
  })
};
